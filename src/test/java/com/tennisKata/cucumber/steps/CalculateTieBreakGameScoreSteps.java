...
public class CalculateTieBreakGameScoreSteps {
	@Autowired
	...
	@Given("^tie break game with id '(\\d+)', firstPlayer set with id '(\\d+)' and secondPlayer set with id '(\\d+)'$")
	public void tie_break_game_with_firstPlayer_and_second_player(final Integer idGame, final Integer idPlayer1,
		...
		Game game = new Game(player1, player2, GameTypeEnum.TIEBREAK);
		game.setId(idGame);
		gameService.save(game);

	}

	@When("^player with id '(\\d+)' win '(\\d+)' points and player with id '(\\d+)' win '(\\d+)' points and calculate score tie break game with id '(\\d+)'$")
	public void win_points_and_calculate_score_of_tie_break_game(final int idPlayer1, final int nbrPoints1, final int idPlayer2,
	...
	@Then("^score of tie break game with id '(\\d+)' is '(.*)'$")
	public void get_score_of_tie_break_game(final int gameId, final String score) {
		Game game = gameService.findById(gameId);
